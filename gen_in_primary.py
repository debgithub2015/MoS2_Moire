...
def create_input_files(iop,xc):
    atoms1 = atoms[0:5:2]
    atoms2 = atoms[1:6:2]
    
    if iop=='bilayer':
	new_atom=atoms1+atoms2
    elif iop=='single-layer':
	new_atom=atoms1
		
    new_coord=new_atom.get_positions()
    posX = new_coord[:,0]
    posY = new_coord[:,1]
    posZ = new_coord[:,2]
    ...
    new_atom.set_cell(cell)
    
    ind_sort = argsort(new_atom.get_chemical_symbols())
    symbols = array(new_atom.get_chemical_symbols())[ind_sort]
    ...
    if iop == 'bilayer':
        ...
        new_comment="single-layer structure"
...
#writing INCAR files
   ...
   ICHARG =      0     charge: 0-INIWAV 1-CHGCAR 2-atom 10-nonself
...
!Electronic Relaxation
   ...
   EDIFF  =   1.0e-7     stopping-criterion for ELM
...
!Ionic relaxation
   ...
   ISIF   =   3     relax atomic coords only
   !TEBEG  =  0.0
...
module load openmpi vasp/6.1.2-knl

srun -n32 -c4 --cpu_bind=cores vasp_std
