...
xc_list = [ 'PBEsol', 'revvdWDF2', 'SCAN+rVV10', 'DFT-D3']

iop=sys.argv[1]

def read_POSCAR(xc):
    infile=os.path.join(os.path.join(basepath,'optimize'),'%s/CONTCAR')%(xc)
...
! Magnetic Moment 
     ...
     MAGMOM = 4*0 8*0  ! local magnetic moment parallel to SAXIS
...
def change_incar_ion_relax(): 
    chng_incar = open("change_incar.sh",'w')
    chng_incar_data='''#! /bin/bash/
...
cp -r OUTCAR relax_fv/OUTCAR_relax_fv
...
sed -i 's/!   ISYM   =      0/   ISYM   =      0/' INCAR
sed -i 's/!   SYMPREC =  1.0e-8/   SYMPREC =  1.0e-8/' INCAR
'''
    chng_incar.write(chng_incar_data)
...
def sc_wsoc():
    chng_sc_wsoc = open("change_sc_wsoc.sh",'w')
    chng_sc_wsoc_data='''#! /bin/bash/

file=relax_ions

[ ! -f $file ] && mkdir $file

cp -r INCAR relax_ions/INCAR_relax_ions
cp -r OUTCAR relax_ions/OUTCAR_relax_ions
cp -r CONTCAR relax_ions/CONTCAR_relax_ions
#cp -r WAVECAR relax_ions/WAVECAR_relax_ions
#cp -r CHGCAR relax_ions/CHGCAR_relax_ions
cp -r vasprun.xml relax_ions/vasprun_relax_ions.xml
cp -r CONTCAR POSCAR

cp -r *.o relax_ions/

python change_sc_input_wsoc.py

'''
    chng_sc_wsoc.write(chng_sc_wsoc_data)
...
cp -r relax_ions/CONTCAR_relax_ions POSCAR
...
#a) Without SOC
def change_input_sc_wsoc():
    chng_sc_inpt_wsoc = open("change_sc_input_wsoc.py",'w')
    chng_sc_inpt_wsoc_data='''import sys
import os

in_file = open("relax_ions/INCAR_relax_ions", "rt")
out_file = open("INCAR", "wt")


for line in in_file:
    line=line.replace('NSW    =    200 ','NSW    = 0')
    line=line.replace('IBRION =    2 ','IBRION =  -1')
...
out_file.close()
'''
    chng_sc_inpt_wsoc.write(chng_sc_inpt_wsoc_data)
...
for line in in_file:
    line=line.replace('NSW    =    200 ','NSW    = 0')
    line=line.replace('IBRION =    2 ','IBRION =  -1')
    line=line.replace('MAGMOM = 4*0 8*0',  'MAGMOM = 0 0 2 0 0 -2 24*0')
...
for line in in_file_kpt:
    line=line.replace('12 12 8','25 25 8')
...
#SBATCH --mail-type=FAIL
#
#SBATCH --comment=1-00:00:00
#SBATCH --time-min=12:0:00
#SBATCH --signal=B:USR1@300
#SBATCH --requeue
#SBATCH --open-mode=append
...
# Prepare for the non self-consistent calculation (without SOC)
sh change_sc_wsoc.sh
...
#writing a description of the output file
    new_comment="bi-layer structure"
...
#writing POTCAR files
    ...
    submit_optimze(atom1,atom2,iop,xc)

    change_input_sc_wsoc()
...
for k in xc_list :
    ...
    Delta=atoms12.get_center_of_mass()-atoms11.get_center_of_mass()
    h=sqrt(sum(Delta**2))
    atoms13=atoms11.copy()
    atoms13.center(about=atoms11.positions[0])
    count=0
    for i in atoms13:
        atoms13.positions[count][2] += 2*h
        count +=1
    atoms14=atoms12.copy()
    atoms14.center(about=atoms12.positions[0])
    count=0
    for i in atoms14:
        atoms14.positions[count][2] += 2*h
        count +=1
    print(atoms13.get_positions(), atoms14.get_positions(), Delta, h)
    atoms_shifted=atoms11+atoms12+atoms13+atoms14
