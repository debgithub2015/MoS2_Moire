...
def read_POSCAR(xc):
    infile=os.path.join(os.path.join(basepath,'sl_optimize'),'%s/CONTCAR')%(xc)
...
! Magnetic Moment 
     ...
     MAGMOM = 1*0 2*0  ! local magnetic moment parallel to SAXIS
...
for line in in_file:
    line=line.replace('NSW    =    200 ','NSW    = 0')
    line=line.replace('IBRION =    2 ','IBRION =  -1')
    line=line.replace('MAGMOM = 1*0 2*0',  'MAGMOM = 0 0 2 0 0 -2 6*0')
...
for line in in_file_kpt:
    line=line.replace('12 12 1','25 25 1')
