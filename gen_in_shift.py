...
if iop == 'shift' : sep_list = [ 0.5, 1.0, 2.0, 5.0 ]
...
def create_input_files(atoms,lattice_scale,cellX,cellY,cellZ,iop,xc,shift):
...
#writing a description of the output file
    new_comment=("%s"+""+"unit shifted bilayer")%(shift)
...
for k in xc_list :
    ...
    atoms22=atoms1[1,4,5]
    Delta = atoms22.get_center_of_mass() - atoms11.get_center_of_mass()
    dist = sqrt(sum(Delta**2))
    unit = Delta/dist
    for i in sep_list:
        shift= i
        new_dir_2= os.path.join(new_dir_1,'%s_unit_shifted')%(i)
        exist_2=os.path.exists(new_dir_2)
        if not exist_2:
            os.mkdir(new_dir_2)
        os.chdir(new_dir_2)
        atoms22.translate((1.0+shift)*unit)
        atoms_shifted=atoms11+atoms22
        create_input_files(atoms_shifted,lat_scale,cell1X,cell1Y,cell1Z,iop,k,i)
